...
3. If the project stores config elsewhere, pass `configPath` explicitly.
...
6. Use `database_explorer_explain_query` for plan inspection and `database_explorer_table_stats` for per-table summary data.
7. Continue as needed with sample, query, describe, or export-schema operations.
8. The tools already return structured output, so consume the tool result directly instead of reparsing plain text.
